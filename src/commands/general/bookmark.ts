            ...
            new SlashCommandBuilder()
                ...
                .addSubcommand(subcommand =>
                    subcommand
                        ...
                        .addNumberOption(option => option.setName("id").setDescription("The id of the bookmark").setRequired(true))
                )
        );
    }
    ...
    private async list(command: ChatInputCommandInteraction) {
        ...
        try {
            const selectMenu = new SelectMenuBuilder()
                .setOptions(
                    filteredMessages.map(message => ({
                        label:
                            `${message.bookmarkId}: ` +
                            ((message.channel as Exclude<TextBasedChannel, DMChannel | PartialDMChannel>).name + " - " + message.author.username).substring(0, 20),
            ...
            const response = await command.editReply({
                embeds: [new EmbedBuilder().setDescription("Select a bookmark to view it.")],
